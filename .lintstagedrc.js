...
module.exports = {
  ...
  "apps/**/*.{js,jsx,ts,tsx}": (filenames) => {
    ...
    return Object.entries(filesByApp).flatMap(([appName, files]) => {
      ...
      return [
        ...
        `bash -c "cd ${appDir} && bunx eslint --fix ${relativeFiles.join(" ")}"`,
        `bash -c "cd ${appDir} && bunx prettier --write ${relativeFiles.join(" ")}"`,
      ]
    })
  },
  ...
  "packages/**/*.{js,jsx,ts,tsx}": (filenames) => {
    ...
    return Object.entries(filesByPackage).flatMap(([pkgName, files]) => {
      ...
      return [
        `bash -c "cd ${pkgDir} && bunx eslint --fix ${relativeFiles.join(" ")}"`,
        `bash -c "cd ${pkgDir} && bunx prettier --write ${relativeFiles.join(" ")}"`,
      ]
    })
  },
